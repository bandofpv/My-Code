...
*/
//Bumper Variables
int leftbutton = 7; //sets leftbutton to pin 8
int rightbutton = 8; //sets rightbutton to pin 12
...
void loop() {
  ...
  int motorDelay180 = 1700; //sets motorDelay180 to 1 second
  int motorDelay90 = 850; //sets 90motrDelay to 600 miliseconds
  ...
  else if (batteryVoltage >= lowBattery && batteryVoltage <= highBattery){
  ...
  rightMotors(motorDelay180); //move left for a second
  ...
  //if the left front sensor doesn't detect an object and the right front sensor detects an object...
   ...
   backwardMotors(motorDelay); //move backwards for a second
   leftMotors(motorDelay90); //move left for a second
