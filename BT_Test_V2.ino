/*
 * This is the final code for my vacuum robot. I now avoids walls and stairs. 
 * It will also stop cleaning when you battery is low preventing damage to the battery. 
 * I also added bluetooth capability so you can control it with you phone or computer
 */
//Bluetooth vairables
char val; //sets val to hold the bluetooth data
...
const int leftgroundsensor = 1; //sets the left ground IR sensor to pin A4
...
int detectgroundDelay = 50; //sets detectground to 50 milliseconds
...
//Fan and Led Blink
void beginDelay(int n, int frequency){ //blink for n seconds at frequency hz
  fanmotorOn();
  for (int i=1; i <= n; i++){
    for(int j=1; j<=frequency; j++){
      digitalWrite(greenLed, HIGH); //Turn red led on
      delay((1000/frequency)/2); //Half time on            
...
boolean detectGround(int groundsensor){
  ...
  delay(detectgroundDelay); //delay for 50 milliseconds
  boolean irValueFourth = digitalRead(groundsensor); //creates irValueFourth to the value of groundsensor
  delay(detectgroundDelay); //delay for 50 milliseconds
  boolean irValueFifth = digitalRead(groundsensor); //creates irValueFourth to the value of groundsensor

  return (irValueFirst ||irValueSecond || irValueThird || irValueFourth || irValueFifth); //returns 1 if one value is one and returns 0 if all values are 0(Bitwise OR)
}

//Voltage Check
float voltageCheck(){
  readBattery(battery); //reads the battery from pin A0
}
//Clean Function
void clean(){
    ...
    int irValue5 = analogRead(leftgroundsensor); //sets irValue5 to the value of the left ground sensor
    int irValue6 = analogRead(rightgroundsensor); //sets irValue 6 to the value of the right ground sensor
    ...
    else if (irValue5 >= 150 && irValue6 <= 150){
      backwardMotors(motorDelay); //move backwards
...
/*
  ...
  */
  ...
  if( Serial.available()) // if data is available to read
  {
    ...
    //if val is equal to w...
    if(val == 'w'){ 
      fanmotorOn(); //turn fan on 
      forwardMotors(motorDelay); //move forwards
      }

    //if val is equal to s...
    else if(val == 's'){
      fanmotorOn(); //turn fan on
      backwardMotors(motorDelay); //move backwards
      }

    //if val is equal to d
    else if(val == 'd') {
      fanmotorOn(); //turn fan on
      rightMotors(turnmotorDelay); //turn right
    }

    //if val is equal to a
    else if(val == 'a') {
      fanmotorOn(); //turn fan on 
      leftMotors(turnmotorDelay); //turn left
    }

    //if val is equal to a space
    else if(val == ' ') {
      stopMotors(); //stop motors
    }

    //if val is equal to q
    else if(val == 'q') {
      stopMotors(); //stop motors
      fanmotorOff(); //turn fan off
    }

    //if val is equal to c
    else if(val == 'c') {
    clean(); //clean
   }
